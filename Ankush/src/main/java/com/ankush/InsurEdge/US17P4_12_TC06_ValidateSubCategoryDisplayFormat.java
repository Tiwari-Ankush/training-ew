...
public class US17P4_12_TC06_ValidateSubCategoryDisplayFormat {
    ...
    public static boolean validation_tc06(WebDriver driver) {
        WebDriverWait wait = getWait(driver);
        ...
        try {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                // Sub Category is column index 5 (0-based)
                String subCat = cells.get(5).getText().trim();

                // Simple readable: not empty and contains at least one letter or digit
                if (subCat.isEmpty() && !subCat.matches(".*[A-Za-z0-9].*")) {
                    System.out.println("fail: row " + (i + 1) + " sub category "+ subCat);
                    return false;
                } else {
                    System.out.println("pass: row " + (i + 1) + " sub category "+ subCat);
                }
            }
            return true;
        } catch (Exception e) {
            System.out.println("validation_tc06 failed: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            if (validation_tc06(driver)) {
                System.out.println("US17P4_12_TC06: PASSED");
            } else {
                System.out.println("US17P4_12_TC06: FAILED");
            }
        } catch (Exception e) {
