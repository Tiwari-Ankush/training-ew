...
public class US17P4_12_TC10_ValidateAppliedOnDateFormat {
    ...
    public static boolean validation_tc10(WebDriver driver) {
        WebDriverWait wait = getWait(driver);
        try {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                String dateText = cells.get(9).getText().trim();

                boolean ok = dateText.matches("\\d{4}-\\d{2}-\\d{2}");
                if (!ok) {
                    System.out.println("fail: row " + (i + 1) + " applied on " + dateText );
                   return false;
                } else {
                    System.out.println("pass: row " + (i + 1) + " applied on "+ dateText);
                }
            }
            return true;

        } catch (Exception e) {
            System.out.println("validation_tc10 failed: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (validation_tc10(driver)) {
                System.out.println("US17P4_12_TC10: PASSED");
            } else {
                System.out.println("US17P4_12_TC10: FAILED");
            }
        } catch (Exception e) {
