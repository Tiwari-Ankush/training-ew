...
public class US17P4_12_TC07_ValidateSumAssuredFormat {
    ...
    public static boolean validation_tc07(WebDriver driver) {
        WebDriverWait wait = getWait(driver);
        try {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                // Sum Assured is column index 6 (0-based)
                String sumAssured = cells.get(6).getText().trim();

                // Must be digits with two decimal places (e.g., 12345.67)
                if (!sumAssured.matches("\\d+\\.\\d{2}")) {
                    System.out.println("fail: row " + (i + 1) + " sum assured " + sumAssured);
                    return false;
                } else {
                    System.out.println("pass: row " + (i + 1) + " sum assured "+ sumAssured);
                }
            }
            ...
            System.out.println("validation_tc07 failed: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (validation_tc07(driver)) {
                System.out.println("US17P4_12_TC07: PASSED");
            } else {
                System.out.println("US17P4_12_TC07: FAILED");
            }
        } catch (Exception e) {
