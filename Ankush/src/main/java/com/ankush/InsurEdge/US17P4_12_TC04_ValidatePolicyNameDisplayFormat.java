...
public class US17P4_12_TC04_ValidatePolicyNameDisplayFormat {
    ...
    public static boolean validation_tc04(WebDriver driver) {
        WebDriverWait wait = getWait(driver);
        try {
            ...
            for (int i = 0; i < limit; i++) {
                ...
                // Policy Name is column index 3 (0-based)
                String policyName = cells.get(3).getText().trim();

                // Simple readable check: not empty and contains at least one letter or digit
                boolean readable = !policyName.isEmpty() && policyName.matches(".*[A-Za-z0-9].*");

                if (!readable) {
                    System.out.println("fail: row " + (i + 1) + " policy name " + policyName);
                    return false;
                } else {
                    System.out.println("pass: row " + (i + 1) + " policy name " + policyName);
                }
            }
            ...
            System.out.println("validation_tc04 failed: " + e.getMessage());
            return false;
        }
    }
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (validation_tc04(driver)) {
                System.out.println("US17P4_12_TC04: PASSED");
            } else {
                System.out.println("US17P4_12_TC04: FAILED");
            }
        } catch (Exception e) {
